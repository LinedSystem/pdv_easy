...
@section('title', 'Editar Cliente - PDV Easy')
...
<div class="max-w-6xl mx-auto space-y-6">
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between pb-5 border-b border-slate-200">
        <div>
            <h1 class="text-2xl font-extrabold text-slate-900 tracking-tight">Editar Cliente</h1>
            <p class="text-sm text-slate-500 font-light mt-1">Atualize as informações do cliente para a gestão de fidelidade, controle de crédito e faturamento.</p>
        </div>
        <div class="mt-4 md:mt-0 flex items-center space-x-3">
            <a href="{{ route('clientes.index') }}" class="px-4 py-2 bg-white border border-slate-200 text-slate-700 rounded-xl text-xs font-semibold hover:bg-slate-50 transition-colors shadow-sm">
                Voltar à Lista
            </a>
        </div>
    </div>
    ...
    @if ($errors->any())
        <div class="p-4 bg-rose-50 border border-rose-100 text-rose-700 rounded-2xl flex items-start space-x-3.5 shadow-sm">
            ...
            <div>
                <h4 class="text-xs font-bold text-rose-950">Não foi possível atualizar o cliente:</h4>
    ...
    <form action="{{ route('clientes.update', $cliente->id) }}" method="POST" class="space-y-6">
        ...
        <!-- Seleção de Tipo de Cliente (Card de Destaque) -->
        <div class="bg-white border border-slate-200/80 rounded-2xl p-6 shadow-sm">
            <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div>
                    <h2 class="text-sm font-bold text-slate-900">Tipo de Personalidade</h2>
                    <p class="text-xs text-slate-500 font-light mt-0.5">Selecione se o cliente é Pessoa Física ou Pessoa Jurídica para adaptar os campos do formulário.</p>
                </div>
                <div class="flex items-center bg-slate-100 p-1 rounded-xl w-fit border border-slate-200/40">
                    <label class="relative flex items-center justify-center px-4 py-2 rounded-lg text-xs font-bold cursor-pointer transition-all duration-200 select-none" id="label-pf">
                        <input type="radio" name="tipo_cliente" value="PF" class="sr-only" 
                            {{ old('tipo_cliente', $cliente->tipo_cliente) === 'PF' ? 'checked' : '' }} onchange="toggleClientType('PF')">
                    ...
                    <label class="relative flex items-center justify-center px-4 py-2 rounded-lg text-xs font-medium text-slate-600 cursor-pointer transition-all duration-200 select-none" id="label-pj">
                        <input type="radio" name="tipo_cliente" value="PJ" class="sr-only" 
                            {{ old('tipo_cliente', $cliente->tipo_cliente) === 'PJ' ? 'checked' : '' }} onchange="toggleClientType('PJ')">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div class="bg-white border border-slate-200/80 rounded-2xl p-6 shadow-sm space-y-5">
                ...
                <div class="space-y-1.5">
                    ...
                    <input type="text" name="documento" id="documento" required placeholder="000.000.000-00" value="{{ old('documento', $cliente->documento) }}"
                ...
                <div class="space-y-1.5">
                    ...
                    <input type="text" name="nome" id="nome" required placeholder="Digite o nome completo" value="{{ old('nome', $cliente->nome) }}"
                ...
                <div class="space-y-1.5">
                    ...
                    <input type="text" name="apelido" id="apelido" placeholder="Apelido ou nome social" value="{{ old('apelido', $cliente->apelido) }}"
                ...
                <div class="space-y-1.5">
                    ...
                    <input type="text" name="registro_geral" id="registro_geral" placeholder="Número do RG" value="{{ old('registro_geral', $cliente->registro_geral) }}"
                ...
                <!-- Grid de Contribuinte & Consumidor -->
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 pt-1">
                    ...
                    <div class="space-y-1.5">
                        ...
                        <select name="tipo_contribuinte" id="tipo_contribuinte"
                            ...
                            <option value="não_contribuinte" {{ old('tipo_contribuinte', $cliente->tipo_contribuinte) === 'não_contribuinte' ? 'selected' : '' }}>Não Contribuinte</option>
                            <option value="contribuinte_icms" {{ old('tipo_contribuinte', $cliente->tipo_contribuinte) === 'contribuinte_icms' ? 'selected' : '' }}>Contribuinte ICMS</option>
                            <option value="isento" {{ old('tipo_contribuinte', $cliente->tipo_contribuinte) === 'isento' ? 'selected' : '' }}>Isento</option>
                        </select>
                    </div>

                    <!-- Consumidor Final Checkbox Toggle -->
                    <div class="space-y-1.5 flex flex-col justify-end">
                        <div class="flex items-center justify-between p-3.5 bg-slate-50 border border-slate-200 rounded-xl transition-all hover:bg-slate-100/50">
                            <div>
                                <span class="text-xs font-bold text-slate-700 block select-none">Consumidor Final</span>
                                <span class="text-[9px] text-slate-400 block select-none">Faturamento simples</span>
                            </div>
                            <label class="inline-flex relative items-center cursor-pointer select-none">
                                <input type="checkbox" name="consumidor_final" id="consumidor_final" class="sr-only peer"
                                    {{ old('consumidor_final', $cliente->consumidor_final ? 'on' : 'off') === 'on' ? 'checked' : '' }}>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div class="space-y-1.5">
                        ...
                        <input type="text" name="telefone_fixo" id="telefone_fixo" placeholder="(00) 0000-0000" value="{{ old('telefone_fixo', $cliente->telefone_fixo) }}"
                    ...
                    <div class="space-y-1.5">
                        ...
                        <input type="text" name="telefone_celular" id="telefone_celular" placeholder="(00) 00000-0000" value="{{ old('telefone_celular', $cliente->telefone_celular) }}"
                ...
                <div class="space-y-1.5">
                    ...
                    <input type="email" name="email" id="email" required placeholder="exemplo@dominio.com" value="{{ old('email', $cliente->email) }}"
            ...
            <div class="bg-white border border-slate-200/80 rounded-2xl p-6 shadow-sm space-y-5">
                ...
                <div class="space-y-1.5">
                    ...
                    <div class="flex items-center gap-2">
                        <input type="text" name="cep" id="cep" required placeholder="00000-000" maxlength="9" value="{{ old('cep', $cliente->cep) }}"
                ...
                <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                    <div class="md:col-span-3 space-y-1.5">
                        ...
                        <input type="text" name="logradouro" id="logradouro" required placeholder="Ex: Rua, Avenida, etc" value="{{ old('logradouro', $cliente->logradouro) }}"
                    ...
                    <div class="md:col-span-1 space-y-1.5">
                        ...
                        <input type="text" name="numero" id="numero" required placeholder="Nº" value="{{ old('numero', $cliente->numero) }}"
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div class="space-y-1.5">
                        ...
                        <input type="text" name="bairro" id="bairro" placeholder="Digite o Bairro" value="{{ old('bairro', $cliente->bairro) }}"
                    ...
                    <div class="space-y-1.5">
                        ...
                        <input type="text" name="complemento" id="complemento" placeholder="Apt, Bloco, etc" value="{{ old('complemento', $cliente->complemento) }}"
                ...
                <div class="grid grid-cols-3 gap-4">
                    <div class="col-span-1.5 space-y-1.5">
                        ...
                        <input type="text" name="cidade" id="cidade" placeholder="Cidade" value="{{ old('cidade', $cliente->cidade) }}"
                    ...
                    <div class="col-span-0.5 space-y-1.5">
                        ...
                        <input type="text" name="uf" id="uf" placeholder="UF" maxlength="2" value="{{ old('uf', $cliente->uf) }}"
                    ...
                    <div class="col-span-1 space-y-1.5">
                        ...
                        <input type="text" name="ibge" id="ibge" placeholder="Cód. IBGE" value="{{ old('ibge', $cliente->ibge) }}"
...
<script>
    ...
    document.addEventListener("DOMContentLoaded", function() {
        const selectedType = "{{ old('tipo_cliente', $cliente->tipo_cliente) }}";
        toggleClientType(selectedType);
    });

    // Alterna o Tipo de Cliente (Pessoa Física / Pessoa Jurídica)
    ...
    docInput.addEventListener('input', function (e) {
        ...
        const isPJ = document.querySelector('input[name="tipo_cliente"]:checked').value === 'PJ';
    ...
    function buscarCep() {
        ...
        if (cep.length !== 8) {
            alert('Por favor, infoame um CEP válido com 8 dígitos.');
            return;
        }
