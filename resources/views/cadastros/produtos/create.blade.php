...
@section('title', 'Cadastrar Produto - PDV Easy')
...
<div class="max-w-6xl mx-auto space-y-6">
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between pb-5 border-b border-slate-200">
        <div>
            <h1 class="text-2xl font-extrabold text-slate-900 tracking-tight">Cadastro de Novo Produto</h1>
            <p class="text-sm text-slate-500 font-light mt-1">Insira as informações do produto, preços de venda, estoque e tributação fiscal.</p>
        </div>
            ...
            <div class="flex flex-col md:flex-row gap-6 items-start">
                ...
                <div class="flex-shrink-0 flex flex-col items-center md:items-start">
                    ...
                    <div id="image-drag-area" style="width: 240px; height: 240px;" class="bg-slate-50 border-2 border-dashed border-slate-200 hover:border-indigo-400 rounded-2xl flex flex-col items-center justify-center p-4 text-center cursor-pointer transition-all relative overflow-hidden group">
                        ...
                        <div id="drag-placeholder" class="space-y-2 group-hover:scale-105 transition-transform duration-200">
                        ...
                        <button type="button" id="remove-image-btn" class="absolute top-1.5 right-1.5 p-1 bg-slate-900/80 hover:bg-slate-900 text-white rounded-lg opacity-0 group-hover:opacity-100 transition-opacity z-10 hidden">
                ...
                <div class="flex-1 w-full space-y-4.5">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                        <div class="md:col-span-2 space-y-1.5">
                            ...
                            <input type="text" name="nome" id="nome" required placeholder="Ex: Refrigerante Lata 350ml" value="{{ old('nome') }}"
                        ...
                        <div class="md:col-span-1 space-y-1.5">
                            ...
                            <input type="text" name="sku" id="sku" placeholder="Ex: REF-LATA-350" value="{{ old('sku') }}"
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div class="space-y-1.5">
                            ...
                            <div class="relative rounded-xl shadow-sm">
                                ...
                                <input type="text" name="barcode" id="barcode" placeholder="Ex: 7891234567890" value="{{ old('barcode') }}"
                        ...
                        <div class="space-y-1.5">
                            ...
                            <input type="text" name="barcode_interno" id="barcode_interno" required placeholder="Ex: 10002" value="{{ old('barcode_interno') }}"
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div class="space-y-1.5">
                            ...
                            <input type="number" step="0.001" name="estoque_inicial" id="estoque_inicial" placeholder="0,000" value="{{ old('estoque_inicial', '0') }}"
                        ...
                        <div class="space-y-1.5">
                            ...
                            <input type="number" step="0.001" name="estoque_minimo" id="estoque_minimo" placeholder="0,000" value="{{ old('estoque_minimo', '0') }}"
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4 bg-slate-50/50 p-4 border border-slate-100 rounded-2xl">
                        <div class="space-y-1.5">
                            ...
                            <input type="number" step="0.01" name="preco_custo" id="preco_custo" placeholder="0,00" value="{{ old('preco_custo', '0.00') }}"
                        ...
                        <div class="space-y-1.5">
                            ...
                            <input type="number" step="0.01" name="margem_lucro" id="margem_lucro" placeholder="0,00" value="{{ old('margem_lucro', '0.00') }}"
                        ...
                        <div class="space-y-1.5">
                            ...
                            <input type="number" step="0.01" name="preco_venda" id="preco_venda" placeholder="0,00" value="{{ old('preco_venda', '0.00') }}"
        ...
        <div class="bg-white border border-slate-200/80 rounded-2xl p-6 shadow-sm space-y-5">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div class="space-y-1.5">
                    ...
                    <select name="tributacao" id="tributacao"
                        ...
                        <option value="Simples Nacional" {{ old('tributacao', 'Simples Nacional') === 'Simples Nacional' ? 'selected' : '' }}>Simples Nacional</option>
                        <option value="Regime Normal" {{ old('tributacao') === 'Regime Normal' ? 'selected' : '' }}>Regime Normal (Lucro Presumido / Real)</option>
                        <option value="Substituição Tributária" {{ old('tributacao') === 'Substituição Tributária' ? 'selected' : '' }}>Substituição Tributária (ST)</option>
                        <option value="Isento / Não Incide" {{ old('tributacao') === 'Isento / Não Incide' ? 'selected' : '' }}>Isento / Não Incidência</option>
                    </select>
                    @error('tributacao')
                        <span class="text-xs font-semibold text-rose-600 block mt-1">{{ $message }}</span>
                    @enderror
                ...
                <div class="space-y-1.5">
                    ...
                    <input type="text" name="ncm" id="ncm" required placeholder="Ex: 2202.10.00" value="{{ old('ncm') }}"
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div class="space-y-1.5">
                    ...
                    <input type="text" name="cest" id="cest" placeholder="Ex: 03.007.00" value="{{ old('cest') }}"
                ...
                <div class="space-y-1.5">
                    ...
                    <select name="origem" id="origem"
                        ...
                        <option value="0" {{ old('origem', '0') === '0' ? 'selected' : '' }}>0 - Nacional</option>
                        ...
                        <option value="3" {{ old('origem') === '3' ? 'selected' : '' }}>3 - Nacional (Mercadoria ou bem com Conteúdo de Importação > 40%)</option>
                        <option value="4" {{ old('origem') === '4' ? 'selected' : '' }}>4 - Nacional (Produção conforme processos produtivos básicos)</option>
                        <option value="5" {{ old('origem') === '5' ? 'selected' : '' }}>5 - Nacional (Conteúdo de Importação <= 40%)</option>
                        <option value="6" {{ old('origem') === '6' ? 'selected' : '' }}>6 - Estrangeira (Importação Direta sem similar nacional)</option>
                        <option value="7" {{ old('origem') === '7' ? 'selected' : '' }}>7 - Estrangeira (Adquirida internamente sem similar nacional)</option>
                    </select>
                    @error('origem')
                        <span class="text-xs font-semibold text-rose-600 block mt-1">{{ $message }}</span>
                    @enderror
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div class="space-y-1.5">
                    ...
                    <input type="text" name="cbenef" id="cbenef" placeholder="Ex: PR800000" value="{{ old('cbenef') }}"
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function calcularMargemLucro() {
        ...
        if (custo > 0) {
            ...
            // Se custo for 0 mas tiver preço de venda, margem é de 100%
