...
@section('title', 'Unidades de Medida - PDV Easy')
...
<div class="max-w-6xl mx-auto space-y-6">
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between pb-5 border-b border-slate-200">
        <div>
            <h1 class="text-2xl font-extrabold text-slate-900 tracking-tight">Auxiliares: Unidades de Medida</h1>
            <p class="text-sm text-slate-500 font-light mt-1">Gerencie as unidades de medida para fracionamento, venda e controle de estoque de produtos.</p>
        </div>
            ...
            <table class="min-w-full divide-y divide-slate-100 text-left">
                <thead>
                    <tr class="bg-slate-50/50">
                        <th scope="col" class="px-6 py-4.5 text-xs font-bold text-slate-400 uppercase tracking-wider">Nome</th>
                        <th scope="col" class="px-6 py-4.5 text-xs font-bold text-slate-400 uppercase tracking-wider">Abreviação</th>
                ...
                <tbody class="divide-y divide-slate-100 bg-white">
                    @if ($unidades->count() > 0)
                        @foreach ($unidades as $unidade)
                            <tr class="hover:bg-slate-50/50 transition-colors {{ $editUnidade && $editUnidade->id === $unidade->id ? 'bg-indigo-50/20' : '' }}">
                                <!-- Coluna Nome -->
                                ...
                                <!-- Coluna Abreviação -->
                                <td class="px-6 py-4 whitespace-nowrap text-xs font-mono font-bold text-slate-500 uppercase">
                                    {{ $unidade->abreviacao }}
                                </td>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-right text-xs font-medium">
                                    <div class="flex items-center justify-end space-x-2">
                                        <!-- Editar (Azul) -->
                                        <button onclick="openEditModal('{{ $unidade->id }}', '{{ addslashes($unidade->nome) }}', '{{ addslashes($unidade->abreviacao) }}')" 
                                        ...
                                        <!-- Excluir (Vermelho) -->
                                        <form action="{{ route('unidades.destroy', $unidade->id) }}" method="POST" class="inline" onsubmit="return confirm('Deseja realmente excluir esta unidade?')">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit" 
                    ...
                    @else
                        <tr>
                            <td colspan="3" class="px-6 py-12 text-center">
                                ...
                                <h4 class="text-xs font-bold text-slate-800">Nenhuma unidade cadastrada ainda</h4>
                                <p class="text-[10px] text-slate-400 mt-1 font-light">Clique no botão acima para cadastrar a primeira unidade.</p>
                            </td>
                        </tr>
                    @endif
                </tbody>
...
<div id="unit-modal" class="fixed inset-0 z-50 hidden items-center justify-center overflow-x-hidden overflow-y-auto outline-none focus:outline-none select-none">
    <!-- Backdrop Overlay -->
    ...
    <div class="relative w-full max-w-md mx-auto my-6 z-50 px-4">
        <div class="relative flex flex-col w-full bg-white border border-slate-200/80 rounded-2xl shadow-2xl outline-none focus:outline-none">
            ...
            <!-- Form -->
            <form id="unit-form" action="{{ route('unidades.store') }}" method="POST" class="space-y-6">
                @csrf
                <div class="p-6 sm:p-8 space-y-6">
                    ...
                    @if ($errors->any())
                        <div class="p-4 bg-rose-50 border border-rose-100 text-rose-700 rounded-xl flex items-start space-x-2.5">
                            ...
                            <span class="text-[10px] font-semibold leading-relaxed block">Erro: Unidade não pôde ser salva. Verifique os dados abaixo.</span>
                        </div>
                    @endif

                    <!-- Input: Nome da Unidade -->
                    <div class="space-y-2">
                        <label for="nome" class="text-xs font-semibold text-slate-700 tracking-wide block">
                            Nome da Unidade <span class="text-rose-500">*</span>
                        </label>
                        <input type="text" name="nome" id="nome" required placeholder="Ex: Quilograma, Unidade..." 
                            value="{{ old('nome') }}"
                    ...
                    <!-- Input: Abreviação -->
                    <div class="space-y-2">
                        <label for="abreviacao" class="text-xs font-semibold text-slate-700 tracking-wide block">
                            Abreviação <span class="text-rose-500">*</span>
                        </label>
                        <input type="text" name="abreviacao" id="abreviacao" required placeholder="Ex: KG, UN, LT..." 
                            maxlength="10"
                            value="{{ old('abreviacao') }}"
                            class="block w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-slate-800 placeholder-slate-400 focus:bg-white focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-600 transition-all duration-200 text-sm font-mono uppercase @error('abreviacao') border-rose-400 focus:ring-rose-500/20 focus:border-rose-500 @enderror">
                        @error('abreviacao')
                            <span id="error-abreviacao" class="text-xs font-semibold text-rose-600 block mt-1">{{ $message }}</span>
                        @enderror
                    </div>
                </div>
...
<script>
    function openCreateModal() {
        ...
        document.getElementById('modal-title').querySelector('span').innerText = 'Cadastrar Unidade';
    ...
    function openEditModal(id, nome, abreviacao) {
        const form = document.getElementById('unit-form');
        ...
        document.getElementById('abreviacao').value = abreviacao;
        document.getElementById('modal-title').querySelector('span').innerText = 'Editar Unidade';
    ...
    function closeModal() {
        ...
        // Remove validation error spans if user cancels
        const errorNome = document.getElementById('error-nome');
        if (errorNome) errorNome.remove();
        
        const errorAbrev = document.getElementById('error-abreviacao');
        if (errorAbrev) errorAbrev.remove();
    }
    ...
    document.addEventListener("DOMContentLoaded", function() {
        @if ($errors->any())
            @if ($editUnidade)
                openEditModal("{{ $editUnidade->id }}", "{{ old('nome', $editUnidade->nome) }}", "{{ old('abreviacao', $editUnidade->abreviacao) }}");
            @else
                openCreateModal();
                // Set prepopulated error values
                document.getElementById('nome').value = "{{ old('nome') }}";
                document.getElementById('abreviacao').value = "{{ old('abreviacao') }}";
            @endif
        @elseif ($editUnidade)
            openEditModal("{{ $editUnidade->id }}", "{{ $editUnidade->nome }}", "{{ $editUnidade->abreviacao }}");
        @endif
    });
</script>
@endsection
