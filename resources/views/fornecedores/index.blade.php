...
@section('title', 'Fornecedores - PDV Easy')
...
<div class="max-w-7xl mx-auto space-y-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-5 border-b border-slate-200">
        <div>
            <h1 class="text-2xl font-extrabold text-slate-900 tracking-tight">Fornecedores</h1>
            <p class="text-sm text-slate-500 font-light mt-1">Gerencie a carteira de fornecedores da sua empresa para compras, controle de estoque e entrada de mercadorias.</p>
        </div>
        <div>
    ...
    <div class="bg-white border border-slate-200/80 rounded-2xl p-4 shadow-sm flex flex-col md:flex-row md:items-center justify-between gap-4">
        <form action="{{ route('fornecedores.index') }}" method="GET" class="w-full md:max-w-md flex items-center gap-2">
            ...
            @if ($search)
                <a href="{{ route('fornecedores.index') }}" class="px-3.5 py-2.5 bg-slate-100 hover:bg-slate-200 text-slate-600 rounded-xl text-xs font-bold transition-all">
                    Limpar
                </a>
            @endif
        ...
        <div class="text-[11px] text-slate-400 font-medium self-center">
            Total de registros: <span class="font-bold text-slate-700">{{ $fornecedores->total() }}</span>
        </div>
    </div>
                ...
                <table class="min-w-full divide-y divide-slate-100 text-left">
                    <thead>
                        <tr class="bg-slate-50/50">
                            <th scope="col" class="px-6 py-4.5 text-xs font-bold text-slate-400 uppercase tracking-wider">Fornecedor</th>
                            ...
                            <th scope="col" class="px-6 py-4.5 text-xs font-bold text-slate-400 uppercase tracking-wider">Status</th>
                    ...
                    <tbody class="divide-y divide-slate-100 bg-white">
                        @foreach ($fornecedores as $fornecedor)
                            <tr class="hover:bg-slate-50/50 transition-colors">
                                <!-- Coluna Fornecedor (Nome + Avatar + Tipo) -->
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="flex items-center space-x-3.5">
                                        ...
                                        <div class="overflow-hidden">
                                            <div class="text-xs font-bold text-slate-800 truncate" title="{{ $fornecedor->nome }}">{{ $fornecedor->nome }}</div>
                                            <div class="flex items-center space-x-1.5 mt-0.5">
                                                ...
                                                @endif
                                                @if($fornecedor->apelido)
                                                    <span class="text-[10px] text-slate-400 truncate">({{ $fornecedor->apelido }})</span>
                                                @endif
                                            </div>
                                        </div>
                                    </div>
                                ...
                                <!-- Coluna Contatos -->
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="text-xs text-slate-700 truncate" title="{{ $fornecedor->email }}">{{ $fornecedor->email }}</div>
                                    <div class="text-[10px] text-slate-400 mt-0.5">
                                        {{ $fornecedor->telefone_celular ?: ($fornecedor->telefone_fixo ?: 'Sem telefone') }}
                                    </div>
                                </td>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    @if($fornecedor->cidade)
                                        <span class="text-xs text-slate-700 font-medium">{{ $fornecedor->cidade }}</span>
                                        <span class="text-[10px] text-slate-400 font-bold bg-slate-100 border border-slate-200/40 px-1 py-0.5 rounded ml-1">{{ $fornecedor->uf }}</span>
                                    @else
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    @if($fornecedor->ativo)
                                        <span class="inline-flex items-center text-[10px] font-bold text-emerald-600 bg-emerald-50 border border-emerald-100/50 px-2.5 py-0.5 rounded-full">
                                            Ativo
                                        </span>
                                    @else
                                        <span class="inline-flex items-center text-[10px] font-bold text-rose-600 bg-rose-50 border border-rose-100/50 px-2.5 py-0.5 rounded-full">
                                            Inativo
                                        </span>
                                    @endif
                                </td>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-right text-xs font-medium">
                                    <div class="flex items-center justify-end space-x-1.5">
                                        <a href="{{ route('fornecedores.edit', $fornecedor->id) }}" class="p-1.5 text-slate-400 hover:text-indigo-600 hover:bg-indigo-50/50 rounded-lg transition-colors cursor-pointer" title="Editar">
                                            <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                                                <path stroke-linecap="round" stroke-linejoin="round" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z" />
                                            </svg>
                                        </a>
                                        <form action="{{ route('fornecedores.destroy', $fornecedor->id) }}" method="POST" class="inline" onsubmit="return confirm('Deseja realmente excluir este fornecedor?')">
                                            @csrf
                                            @method('DELETE')
        ...
        <div class="bg-white border border-slate-200/80 rounded-3xl p-12 text-center shadow-sm space-y-6 max-w-xl mx-auto my-8">
            ...
            <div class="space-y-2">
                <h3 class="text-base font-bold text-slate-900">
                    @if ($search)
                        Nenhum fornecedor correspondente encontrado
                    @else
                ...
                <p class="text-xs text-slate-500 font-light leading-relaxed max-w-md mx-auto">
                    ...
                    @else
                        Cadastre seus fornecedores para registrar notas fiscais de entrada, controlar os custos das compras e repor itens do seu estoque.
                    @endif
                </p>
            </div>
            ...
            <div class="flex items-center justify-center gap-3">
                @if ($search)
                    <a href="{{ route('fornecedores.index') }}" class="px-4 py-2.5 bg-slate-100 hover:bg-slate-200 text-slate-700 rounded-xl text-xs font-semibold transition-all">
                        Limpar Busca
                    </a>
                @else
                    <a href="{{ route('fornecedores.create') }}" class="px-5 py-2.5 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl text-xs font-semibold shadow-lg shadow-indigo-100 transition-all">
